...
test('R02 release registry covers and conservatively decides every canonical resource', () => {
  ...
  assert.equal(registry.objects.find((object) => object.id === 'policy:rights-attribution-policy').lastReviewedOrTested, '2026-08-02');
  ...
  assert.equal(registry.objects.find((object) => object.id === 'policy:rights-attribution-policy').governanceControls.staleness.reviewBy, '2026-11-02');
...
test('R07 records five bounded tool evaluations plus seven bounded site-assurance domains without overstating certification', () => {
  ...
  const rightsAttribution = assurance.siteAssuranceDomains.find((item) => item.id === 'rights-attribution');
  assert.equal(rightsAttribution.state, 'passed-limited');

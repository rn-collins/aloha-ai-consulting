...
test('R02 release registry covers and conservatively decides every canonical resource', () => {
  ...
  assert.equal(registry.counts.objects, 260);
  assert.equal(registry.objects.length, 260);
  assert.equal(manifest.objects.length, 260);
  assert.equal(new Set(registry.objects.map((object) => object.id)).size, 260);
  ...
  assert.ok(registry.objects.filter((object) => !['tool:claims-checker', 'tool:evidence-explainer', 'tool:bill-analyzer', 'tool:controlled-substances-explainer', 'policy:accessibility-statement', 'policy:corrections-policy', 'policy:legal-authority-policy'].includes(object.id)).every((object) => object.lastReviewedOrTested === '2026-07-31'));
  ...
  assert.equal(registry.objects.find((object) => object.id === 'policy:corrections-policy').lastReviewedOrTested, '2026-08-02');
  assert.equal(registry.objects.find((object) => object.id === 'policy:legal-authority-policy').lastReviewedOrTested, '2026-08-02');
  ...
  assert.equal(registry.objects.find((object) => object.id === 'policy:corrections-policy').governanceControls.staleness.reviewBy, '2026-11-02');
  assert.equal(registry.objects.find((object) => object.id === 'policy:legal-authority-policy').governanceControls.staleness.reviewBy, '2026-11-02');
...
test('R07 records five bounded tool evaluations plus five bounded site-assurance domains without overstating certification', () => {
  ...
  const accessibilityEvaluation = JSON.parse(fs.readFileSync('api/evaluations/accessibility.json', 'utf8'));
  ...
  const bill = assurance.highStakesEvaluationQueue.find((item) => item.canonicalId === 'bill-analyzer');
  assert.equal(bill.state, 'passed-limited');
  assert.match(bill.decision, /regulatory-language triage/);
  const controlledSubstances = assurance.highStakesEvaluationQueue.find((item) => item.canonicalId === 'controlled-substances-explainer');
  assert.equal(controlledSubstances.state, 'passed-limited');
  assert.match(controlledSubstances.decision, /four configured substance names/);
  ...
  assert.equal(claimsEvaluation.metrics.totalCases, 30);
  assert.equal(claimsEvaluation.metrics.failedCases, 0);
  ...
  assert.equal(billEvaluation.metrics.totalCases, 30);
  assert.equal(billEvaluation.metrics.failedCases, 0);
  assert.equal(billEvaluation.metrics.falseClearances, 0);
  ...
  assert.equal(controlledSubstancesEvaluation.metrics.totalCases, 24);
  assert.equal(controlledSubstancesEvaluation.metrics.failedCases, 0);
  assert.equal(controlledSubstancesEvaluation.metrics.falseClearances, 0);
  assert.equal(controlledSubstancesEvaluation.metrics.abstentionAccuracy, 1);
  assert.equal(controlledSubstancesEvaluation.metrics.unsupportedInferenceRejections, 1);
  assert.equal(controlledSubstancesEvaluation.decision, 'passed-limited-federal-authority-routing-scope');
  ...
  const accessibility = assurance.siteAssuranceDomains.find((item) => item.id === 'accessibility');
  assert.equal(accessibility.state, 'passed-limited');
  assert.equal(accessibility.evidenceHref, '/api/evaluations/accessibility.json');
  assert.equal(accessibilityEvaluation.decision, 'passed-limited-static-structure-and-interaction-contract-scope');
  assert.equal(accessibilityEvaluation.metrics.failedChecks, 0);
  assert.equal(accessibilityEvaluation.metrics.structuralFindings, 0);
  assert.equal(accessibilityEvaluation.assistiveTechnologyEvidence.performed.length, 0);
  assert.ok(accessibilityEvaluation.assistiveTechnologyEvidence.notPerformed.length >= 4);
  const corrections = assurance.siteAssuranceDomains.find((item) => item.id === 'corrections');
  assert.equal(corrections.state, 'passed-limited');
  assert.equal(corrections.evidenceHref, '/api/evaluations/corrections.json');
  assert.equal(correctionsEvaluation.decision, 'passed-limited-public-correction-process-and-ledger-integrity-scope');
  assert.equal(correctionsEvaluation.metrics.failedChecks, 0);
  assert.ok(correctionsEvaluation.revisionLedger.entries >= 2);
  assert.ok(correctionsEvaluation.affectedOutputAnalysis.totalDispositionedOutputs >= 6);
  const legalAuthority = assurance.siteAssuranceDomains.find((item) => item.id === 'legal-authority');
  assert.equal(legalAuthority.state, 'passed-limited');
  assert.equal(legalAuthority.evidenceHref, '/api/evaluations/legal-authority.json');
  assert.equal(legalAuthorityEvaluation.decision, 'passed-limited-selected-authority-control-scope');
  assert.equal(legalAuthorityEvaluation.metrics.failedChecks, 0);
  assert.ok(legalAuthorityEvaluation.sourceRegister.records >= 2);
  assert.ok(assurance.siteAssuranceDomains.filter((item) => !['privacy','security','accessibility','corrections','legal-authority'].includes(item.id)).every((item) => item.state === 'required-not-yet-certified' && item.requiredEvidence));
  assert.equal(manifest.counts.methodControls, 12);
  assert.equal(manifest.counts.evaluatedHighStakesTools, 5);
  assert.equal(manifest.counts.evaluatedAssuranceDomains, 5);

...
check('R10-30', evidence?.reAudit?.confirmedSemanticSuccessors + evidence?.reAudit?.confirmedRemediationDispositions + evidence?.reAudit?.confirmedRouteTargetSuccessors + evidence?.reAudit?.lineageUnresolved === 1079, 'Every non-verbatim promise must reconcile to one supported lineage class or an unresolved decision.');
...
check('R10-32', evidence?.reAudit?.confirmedRouteTargetSuccessors === 667 && evidence?.reAudit?.lineageUnresolved === 173, 'Unique route-target continuity must confirm exactly 667 successors while preserving 173 unresolved lineage decisions.');
check('R10-33', evidence?.reAudit?.confirmedSemanticSuccessors + evidence?.reAudit?.confirmedRemediationDispositions + evidence?.reAudit?.confirmedRouteTargetSuccessors + evidence?.reAudit?.lineageUnresolved === 1079, 'Every non-verbatim promise must reconcile to one supported lineage class or remain unresolved.');

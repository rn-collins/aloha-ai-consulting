...
const evaluation = read('content/evaluations/evidence-explainer.json');
const tools = read('content/tools/operational-tools.json');
const tool = tools.find((item) => item.id === evaluation.canonicalId);
if (!tool) throw new Error('Evidence Explainer canonical tool record is missing.');
const signals = tool.demo?.signals || [];
if (tool.version !== evaluation.toolVersion || signals.length !== 5) throw new Error('Evidence Explainer rules diverge from evaluated five-category contract 1.1.0.');
if (signals.some((item) => !item.evidenceQuestion || !item.evidenceDesign || !Array.isArray(item.confounders) || !item.confounders.length)) throw new Error('Evidence Explainer output contract is incomplete.');
...
const report = {
  ...Object.fromEntries(['schema','evaluationId','canonicalId','toolVersion','methodVersion','corpusVersion','evaluatedAt','owner','reviewer','scope','prohibitedInference','decisionThreshold'].map((key) => [key, evaluation[key]])),
  ...
  decision: thresholdPassed ? 'passed-limited-claim-language-triage-scope' : 'rejected',
...
fs.writeFileSync('api/evaluations/evidence-explainer.json', `${JSON.stringify(report, null, 2)}\n`);
if (!thresholdPassed || failures.length) {
  console.error(`Evidence Explainer evaluation rejected: ${failures.length} case failures; ${falseClearances} false clearances; ${unexpectedCategoryFlags} unexpected flags.`);
  process.exit(1);
}
console.log(`Evidence Explainer bounded evaluation passed: ${results.length}/${results.length} cases; ${correctAbstentions}/${abstentionCases.length} abstentions; ${evaluation.unsupportedInferenceCases.length}/${evaluation.unsupportedInferenceCases.length} unsupported inferences rejected.`);

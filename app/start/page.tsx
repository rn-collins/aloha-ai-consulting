...
const choices=[
 ...
 ["07","I want a year of governed AI decision learning","Inspect the twelve-month Decision Desk program, its twelve distinct questions and instruments, and the truthful readiness of the flagship and future issues.","/learning/decision-desk","Open Decision Desk"],
 ["08","I may want to work with Aloha AI","See what is available now, what remains under validation, and how to prepare a useful inquiry without encountering a false booking path.","/work","Explore ways to work together"],
];
export default function Start(){return <SiteShell><main id="main"><section className="page-hero wrap"><p className="kicker">Start here</p><h1>Choose by situation,<br/><em>not by software.</em></h1><p className="lede">Four browser-local decision tools, two complete flagship courses, and the governed twelve-month Decision Desk architecture are public now. Each page states what is complete, what remains gated, and what it cannot establish.</p></section><section className="wrap"><div className="choice-grid">{choices.map(c=><article className="choice" key={c[0]}><span className="path-num">{c[0]}</span><h2>{c[1]}</h2><p>{c[2]}</p><Link href={c[3]}>{c[4]} →</Link></article>)}</div><aside className="quiet-note"><h2>“Do not proceed” is a useful result.</h2><p>These tools are designed to reveal missing evidence, authority, safeguards, or capacity. They do not manufacture a positive recommendation when the conditions do not support one.</p></aside></section></main></SiteShell>}

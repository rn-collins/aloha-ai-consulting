...
export function expandEducationResources(resources) {
  ...
  for (const courseResource of resources.filter((item) => item.kind === 'course' && item.education?.records?.length)) {
    ...
    orderedLessons.forEach(({ lesson, module }, index) => {
      const lessonSlug = (value) => value.replace(/^[a-z0-9-]+-lesson-/, '');
      const slug = lessonSlug(lesson.id);
      ...
      expanded.push({
        ...
        parentCourse: {
          ...
          previousPathname: index ? `${courseResource.pathname}/lessons/${lessonSlug(orderedLessons[index - 1].lesson.id)}` : null,
          nextPathname: index < orderedLessons.length - 1 ? `${courseResource.pathname}/lessons/${lessonSlug(orderedLessons[index + 1].lesson.id)}` : null
        },

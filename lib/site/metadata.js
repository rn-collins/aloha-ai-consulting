...
const DEFAULT_IMAGE = `${ORIGIN}/og-image.png?v=20260729b`;
const DEFAULT_IMAGE_ALT = 'Aloha AI — Complex work, made usable. An interdisciplinary AI practice by RN Collins.';
...
export function decodeHtmlText(value) {
  return String(value ?? '').replace(/&(?:#(\d+)|#x([0-9a-f]+)|amp|quot|apos|#039|lt|gt);/gi, (entity, decimal, hexadecimal) => {
...
export function renderMetadata(meta) {
  return [
    ...
    `<meta property="og:image:width" content="${escapeHtml(meta.openGraph.imageWidth)}">`,
    `<meta property="og:image:height" content="${escapeHtml(meta.openGraph.imageHeight)}">`,

...
function renderSearch() {
  ...
  const body = `<section class="search-cover"><div class="wrap wrap--wide"><div><p class="eyebrow">Search the whole system</p><h1>What are you trying<br>to <em>do?</em></h1><p>Use ordinary language. Search covers services, tools, research, monitors, products, and every University resource.</p></div><aside aria-hidden="true"><span>ASK</span><span>SCAN</span><span>OPEN</span><b>139<br><small>canonical resources</small></b></aside></div></section>
...
function searchClientScript() {
  ...
  function run(query,push){
    ...
    if(!ready){status.textContent='Loading the resource index…';return;}
    render(query);
  }
  ...
  fetch('/search-index.json').then(function(response){if(!response.ok)throw new Error('Search index unavailable');return response.json();})
    ...
    .catch(function(){status.className='search-status is-error';status.textContent='Search is temporarily unavailable.';results.setAttribute('aria-busy','false');var error=add('div','search-state search-state--error');error.append(add('h2','','The index did not load. The rest of the site still works.'));var browse=add('a','btn btn--outline','Browse topics');browse.href='/topics';error.append(browse);var contact=add('a','btn btn--primary','Start a conversation');contact.href='/university/contact';error.append(contact);results.append(error);});
})();`;
}
...
<div class="not-found__copy"><p class="eyebrow">This route does not exist</p><h1>The page moved—or the path never belonged here.</h1><p>Nothing is hidden behind this error. Search the complete resource system, return home, or choose a reliable front door.</p><form class="not-found__search" action="/search" method="get" role="search"><label for="not-found-query">Search Aloha AI</label><div><input id="not-found-query" name="q" type="search" placeholder="What were you looking for?"><button class="btn btn--primary" type="submit">Search</button></div></form><nav aria-label="Useful destinations"><a href="/">Home</a><a href="/services">Services</a><a href="/tools">Tools</a><a href="/university">University</a><a href="/methods">Methods</a></nav></div>
  ...
  return page('Page not found', 'The requested Aloha AI page does not exist. Search the platform or choose a reliable destination.', body, 'not-found', '/404', 'is-not-found')

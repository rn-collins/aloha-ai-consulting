...
const RELATIONS = ['uses','supports','teaches','implements','evidences','depends_on','replaces','extends','produced_by','available_in_workspace','requires','supersedes','documents','related_to'];
...
export function derivePlatform(resources) {
  ...
  return { registry, collections, topics, audiences, industries, maturity, graph };
}
...
export function validatePlatform(resources, platform) {
  ...
  for (const r of resources) {
    ...
    if (!rels.length) warnings.push(`${r.id}: orphan candidate has no relationships`);
  ...
  warnings.push(...reachabilityWarnings(resources, platform.graph));
  return { errors, warnings };
}

export function generatedOutputs(resources, platform) {
  const outputs = new Map();
  for (const [name, items] of platform.collections) outputs.set(`/${name}`, renderCollection(name, items, `${label(name)} resources`));
  for (const [key, items] of platform.topics) outputs.set(`/topics/${key}`, renderCollection(`topic-${key}`, items, `Topic: ${humanize(key)}`));
  for (const [key, items] of platform.audiences) outputs.set(`/audiences/${key}`, renderCollection(`audience-${key}`, items, `For ${humanize(key)}`));
  for (const [key, items] of platform.industries) outputs.set(`/industries/${key}`, renderCollection(`industry-${key}`, items, `Industry: ${humanize(key)}`));
  for (const [key, items] of platform.maturity) outputs.set(`/maturity/${key}`, renderCollection(`maturity-${key}`, items, `${humanize(key)} resources`));
  outputs.set('/topics', renderDirectory('Topics', platform.topics, '/topics/'));
  outputs.set('/audiences', renderDirectory('Audiences', platform.audiences, '/audiences/'));
  outputs.set('/industries', renderDirectory('Industries', platform.industries, '/industries/'));
  outputs.set('/maturity', renderDirectory('Maturity', platform.maturity, '/maturity/'));
  outputs.set('/api/resources.json', JSON.stringify(apiPayload(resources), null, 2));
  ...
  outputs.set('/workspace/resource-registry.json', JSON.stringify(workspaceRegistry(resources), null, 2));
...
function apiPayload(resources) { return { version: 1, generatedAt: new Date().toISOString(), count: resources.length, resources: resources.map(publicResource) }; }
function publicResource(r) { const { sourceFile, ...publicFields } = r; return { ...publicFields, workspaceUrl: workspaceUrl(r), assessmentUrl: assessmentUrl(r) }; }
...
function workspaceRegistry(resources) { return resources.map((r) => ({ resourceId:r.id, pathname:r.pathname, kind:r.kind, maturity:r.maturity, workspaceUrl:workspaceUrl(r), assessmentUrl:assessmentUrl(r), relationships:relationships(r) })); }
function workspaceUrl(r) { return `/workspace?resource_id=${encodeURIComponent(r.id)}&source_path=${encodeURIComponent(r.pathname)}`; }
function assessmentUrl(r) { return `/tools/ai-readiness-scorecard?resource_id=${encodeURIComponent(r.id)}&source_path=${encodeURIComponent(r.pathname)}`; }
...
function relationships(r) { return r.relationships || (r.relatedIds||[]).map((target)=>({type:'related_to',target})); }
...
function cycleErrors(resources, registry) { const errors=[]; const visiting=new Set(), visited=new Set(); const dfs=(id,trail=[])=>{if(visiting.has(id)){errors.push(`dependency cycle: ${[...trail,id].join(' -> ')}`);return;} if(visited.has(id))return; visiting.add(id); const r=registry.get(id); for(const rel of relationships(r||{}).filter((x)=>['depends_on','requires'].includes(x.type))) if(registry.has(rel.target)) dfs(rel.target,[...trail,id]); visiting.delete(id); visited.add(id);}; for(const r of resources) dfs(r.id); return [...new Set(errors)]; }
function reciprocityWarnings(resources, registry) { const out=[]; for(const r of resources) for(const rel of relationships(r)) if(['replaces','supersedes','extends'].includes(rel.type)&&registry.has(rel.target)&&!relationships(registry.get(rel.target)).some((x)=>x.target===r.id)) out.push(`${r.id}: ${rel.type} ${rel.target} has no reciprocal edge`); return out; }
function reachabilityWarnings(resources, graph) { const inbound=new Map(resources.map((r)=>[r.id,0])); for(const e of graph.edges) inbound.set(e.target,(inbound.get(e.target)||0)+1); return resources.filter((r)=>!relationships(r).length && !inbound.get(r.id)).map((r)=>`${r.id}: unreachable orphan resource`); }

...
const COLLECTION_EXPERIENCES = Object.freeze({
  ...
  research: {
    ...
    title: 'Read the thinking. Then inspect what it became.',
    summary: 'Research notes about where AI systems fail, how trust breaks, and the concrete design choices that make consequential work easier to check.',
    prompt: 'Start with the question closest to a problem you are trying to solve.',
...
<div class="collection-cover__poster" aria-hidden="true"><span>${esc(id)}</span><strong>${String(ordered.length).padStart(2, '0')}</strong><p>things to<br>open, question<br>& inspect</p><i>ALOHA AI ↗</i></div>
</div></section>
<section class="collection-index" id="collection-index"><div class="wrap wrap--wide">
<div class="collection-index__head"><div><p class="eyebrow">${esc(experience.issue)}</p><h2>Pick one.<br>Follow the question.</h2></div><p>${esc(experience.prompt)} Every entry tells you what it is and how far along it is before you open it.</p></div>
...
<span class="collection-entry__top"><span>${String(index + 1).padStart(2, '0')}</span><span>${esc(maturityLabel(resource.maturity))}</span></span>
<span class="collection-entry__kind">${esc(label(resource.kind))}</span>
...
function renderDirectory(title, index, prefix, canonicalPath) {
  ...
  const prompt = ({
    Topics: 'Use this index when you already know the subject you want to explore.',
    Audiences: 'Use this index when you want to see what was made for a particular person or team.',
    Industries: 'Use this index when you already know the field or market you care about.',
    Maturity: 'Use this index when you want to separate live work, public betas, and research.'
  })[title] || `Use this index when you already know how you want to browse ${title.toLowerCase()}.`;
  return page(title, `Browse Aloha AI by ${title.toLowerCase()}.`, `<section class="directory-cover"><div class="wrap wrap--wide"><p class="eyebrow">Another way into Aloha AI</p><h1>${esc(title)}</h1><p>${esc(prompt)} If you do not, <a href="/search">describe your question instead</a>.</p><span>${entries.length} paths</span></div></section><section class="directory-index"><div class="wrap wrap--wide"><div class="directory-index__grid">${entries.map(([key, items], index) => `<a class="directory-entry" href="${prefix}${key}"><span>${String(index + 1).padStart(2, '0')}</span><strong>${esc(humanize(key))}</strong><small>${items.length} resource${items.length === 1 ? '' : 's'} ↗</small></a>`).join('')}</div></div></section>`, `directory-${slug(title)}`, canonicalPath, 'is-directory');
}

function page(title, description, body, id, canonicalPath, bodyClass = '') {

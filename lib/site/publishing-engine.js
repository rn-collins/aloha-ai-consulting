...
function renderCollectionBlock(block) {
  ...
  if (block.type === 'table') {
    ...
    return `<div style="overflow-x:auto"><table><tbody>${normalized.map((cells, rowIndex) => `<tr>${cells.map((cell) => `<${cell.header || rowIndex === 0 ? 'th' : 'td'}>${esc(cell.text)}</${cell.header || rowIndex === 0 ? 'th' : 'td'}>`).join('')}</tr>`).join('')}</tbody></table></div>`;
  }
  return '';
}

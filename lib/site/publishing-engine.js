...
export function generatedOutputs(resources, platform) {
  ...
  for (const [name, items] of platform.collections) outputs.set(`/${name}`, renderCollection(name, items, `${label(name)} resources`, `/${name}`));
...
function apiPayload(resources) { return { version: 1, generatedAt: deterministicTimestamp(resources), count: resources.length, resources: resources.map(publicResource) }; }

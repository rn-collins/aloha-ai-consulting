...
export function generatedOutputs(resources, platform, collectionPages = {}, supplementalRoutes = []) {
  ...
  outputs.set('/sitemap.xml', sitemap(resources, outputs, supplementalRoutes));
...
function sitemap(resources, outputs, supplementalRoutes = []) { const routes = new Set([...resources.map((r)=>r.pathname), ...supplementalRoutes, ...[...outputs.keys()].filter((p)=>p!=='/404'&&!p.endsWith('.json')&&!p.endsWith('.txt')&&!p.endsWith('.xml'))]); routes.delete('/404'); return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${[...routes].sort().map((r)=>`  <url><loc>${BASE_URL}${r}</loc></url>`).join('\n')}\n</urlset>\n`; }

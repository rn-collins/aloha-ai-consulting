...
<script src="/site-shell.js" defer></script>
  ...
  const accessiblePage = page
    .replace('id="demo-risk" style=', 'id="demo-risk" aria-label="Text to screen for guardrail violations" style=')
    .replace('id="demo-review" style=', 'id="demo-review" aria-label="Draft awaiting human review" style=')
    .replace('id="demo-status" class="muted"', 'id="demo-status" class="muted" role="status" aria-live="polite"');
  if (resource.pathname === '/university/contact') return accessiblePage;
  const governedContact = escapeHtml(conversionHref(resource, resource.kind === 'service' ? serviceBranch(resource) : 'not-sure'));
  return accessiblePage.replaceAll('href="/university/contact"', `href="${governedContact}"`);
}
...
function renderEditorialBlocks(blocks, linkText = linkKnownResources) {
  return blocks.map((block) => {
    ...
    if (block.type === 'table') {
      ...
      return `<div style="overflow-x:auto"><table><caption class="visually-hidden">${escapeHtml(caption)}</caption><thead><tr>${head.map((cell) => `<th scope="col">${linkText(cell.text)}</th>`).join('')}</tr></thead><tbody>${normalized.slice(1).map((cells) => `<tr>${cells.map((cell) => cell.header ? `<th scope="row">${linkText(cell.text)}</th>` : `<td>${linkText(cell.text)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
    }
    return '';
  }).join('');
}
...
function governedStatusLabel(resource) {
  ...
  if (resource.kind === 'template' && status.interaction === 'working' && status.access === 'public') return 'Browser-local downloadable template';
...
function browserToolSectionV2(r) {
  ...
  const ruleVersion = ['claims-checker', 'evidence-explainer', 'bill-analyzer', 'controlled-substances-explainer'].includes(r.id) ? '1.1.0' : '1.0.0';
  if (r.id === 'claims-checker') return claimsCheckerSection(r, contract, ruleVersion);
  if (r.id === 'evidence-explainer') return evidenceExplainerSection(r, contract, ruleVersion);
  if (r.id === 'bill-analyzer') return billAnalyzerSection(r, contract, ruleVersion);
  if (r.id === 'controlled-substances-explainer') return controlledSubstancesSection(r, contract, ruleVersion);
...
function controlledSubstancesSection(r, contract, ruleVersion) {
  return `<section class="section"><div class="wrap"><p class="eyebrow">Browser-local federal authority router</p><h2 class="h2">Route a configured substance to dated federal sources.</h2><p class="muted">Deterministic rule version ${ruleVersion}. Federal-only source routing for four configured substances; not a legal-status or activity determination.</p><div class="grid grid-2"><form id="browser-tool-form" class="card" novalidate><label for="browser-tool-input"><strong>${escapeHtml(r.demo.inputLabel)}</strong></label><textarea id="browser-tool-input" style="width:100%;min-height:14rem"></textarea><p><button class="btn btn--primary" type="submit">Route federal sources</button> <button class="btn btn--outline" type="button" id="browser-tool-sample">Load sample</button> <button class="btn btn--ghost" type="reset">Clear</button></p><div id="browser-tool-errors" class="card" role="alert" hidden></div></form><div class="card"><p class="eyebrow">Routing record</p><p id="browser-tool-status" class="muted" role="status" aria-live="polite">No routing record generated.</p><div id="browser-tool-output" hidden aria-live="polite"></div><p><button id="browser-tool-copy" class="btn btn--ghost" type="button" disabled>Copy record</button> <button id="browser-tool-json" class="btn btn--outline" type="button" disabled>Download JSON</button></p></div></div></div></section><script>(function(){var c=${contract},form=document.getElementById('browser-tool-form'),input=document.getElementById('browser-tool-input'),out=document.getElementById('browser-tool-output'),copy=document.getElementById('browser-tool-copy'),download=document.getElementById('browser-tool-json'),record=null;if(!form||!input||!out)return;function esc(v){return window.AlohaState.escapeHtml(v);}function words(v){return ' '+String(v).toLowerCase().replace(/[^a-z0-9]+/g,' ').trim()+' ';}function found(text,term){return words(text).includes(words(term));}function disable(v){copy.disabled=v;download.disabled=v;}function run(){var shell=window.AlohaState.create({form:form,result:out,errors:'browser-tool-errors',status:'browser-tool-status'}),text=input.value.trim();if(!text){record=null;disable(true);return shell.invalid(['Name a substance or question before routing.']);}var matches=(c.records||[]).filter(function(item){return item.aliases.some(function(alias){return found(text,alias);});});record={schema:'aloha-ai-federal-authority-routing/1.0',ruleVersion:${JSON.stringify(ruleVersion)},generatedAt:new Date().toISOString(),input:text,jurisdiction:'United States federal only',matches:matches.map(function(item){return{label:item.label,schedule:item.schedule,asOf:item.asOf,authorityHref:item.authorityHref,federalRegisterHref:item.federalRegisterHref,verification:item.verification};}),abstained:matches.length===0,notice:'Source routing only. This does not determine legality, permission, approval, registration eligibility, applicable requirements, or nonfederal law.'};var body=matches.length?'<h3>'+matches.length+' configured federal record'+(matches.length===1?'':'s')+' routed</h3>'+matches.map(function(item){return'<article><h4>'+esc(item.label)+'</h4><p><strong>'+esc(item.schedule)+'</strong> · as of '+esc(item.asOf)+'</p><p><a href="'+esc(item.authorityHref)+'">Open current eCFR source</a> · <a href="'+esc(item.federalRegisterHref)+'">Open Federal Register route</a></p><p>'+esc(item.verification)+'</p></article>';}).join(''):'<h3>No configured substance matched.</h3><p>The tool abstained. It does not infer that the substance is uncontrolled, legal, or outside regulation.</p>';shell.complete(body+'<p class="muted">'+esc(record.notice)+'</p>','Federal source-routing record generated. Verify every source before use.');disable(false);}form.addEventListener('submit',function(e){e.preventDefault();run();});form.addEventListener('reset',function(){setTimeout(function(){record=null;disable(true);window.AlohaState.create({form:form,result:out,errors:'browser-tool-errors',status:'browser-tool-status'}).reset();},0);});document.getElementById('browser-tool-sample').onclick=function(){input.value=c.sample||'';run();};copy.onclick=function(){if(record)window.AlohaActions.copyText(JSON.stringify(record,null,2),{success:'Routing record copied.'});};download.onclick=function(){if(record)window.AlohaActions.download({content:JSON.stringify(record,null,2),filename:'controlled-substances-federal-routing-v1.json',mimeType:'application/json;charset=utf-8',version:${JSON.stringify(ruleVersion)},license:'Source-routing record; not legal advice or a legal-status determination.'});};disable(true);})();</script>`;
}

function billAnalyzerSection(r, contract, ruleVersion) {
  return `<section class="section"><div class="wrap"><p class="eyebrow">Interactive tool</p><h2 class="h2">${escapeHtml(r.demo.outputTitle)}</h2><p class="muted">Deterministic rule version ${ruleVersion}. English-language lexical triage; input is processed only in this browser.</p><noscript><div class="card"><strong>JavaScript is required to run this browser-local tool.</strong></div></noscript><div class="grid grid-2"><form id="browser-tool-form" class="card" novalidate><label for="browser-tool-input"><strong>${escapeHtml(r.demo.inputLabel)}</strong></label><textarea id="browser-tool-input" style="width:100%;min-height:14rem"></textarea><p><button class="btn btn--primary" type="submit">Run locally</button> <button class="btn btn--outline" type="button" id="browser-tool-sample">Load sample</button> <button class="btn btn--ghost" type="reset">Clear</button></p><div id="browser-tool-errors" class="card" role="alert" hidden></div></form><div class="card"><p class="eyebrow">Structured output</p><p id="browser-tool-status" class="muted" role="status" aria-live="polite">No result has been generated.</p><div id="browser-tool-output" hidden aria-live="polite"></div><p><button id="browser-tool-copy" class="btn btn--ghost" type="button" disabled>Copy result</button> <button id="browser-tool-md" class="btn btn--outline" type="button" disabled>Download Markdown</button> <button id="browser-tool-json" class="btn btn--outline" type="button" disabled>Download JSON</button></p></div></div></div></section><script>(function(){var c=${contract},version=${JSON.stringify(ruleVersion)},form=document.getElementById('browser-tool-form'),input=document.getElementById('browser-tool-input'),out=document.getElementById('browser-tool-output'),record=null;function esc(v){return window.AlohaState.escapeHtml(v);}function disable(v){['browser-tool-copy','browser-tool-md','browser-tool-json'].forEach(function(id){document.getElementById(id).disabled=v;});}function found(text,term){if(term==='$')return text.indexOf('$')!==-1;var escaped=String(term).replace(/[.*+?^\${}()|[\]\\]/g,'\\$&'),left=/^[a-z0-9]/i.test(term)?'\\b':'',right=/[a-z0-9]$/i.test(term)?'\\b':'';return new RegExp(left+escaped+right,'i').test(text);}function run(){var shell=window.AlohaState.create({form:form,result:out,errors:'browser-tool-errors',status:'browser-tool-status'}),text=input.value.trim();if(!text)return shell.invalid(['Enter bill or regulatory text before running the tool.']);var sections=(c.signals||[]).map(function(signal){return{label:signal.label,reviewQuestion:signal.reviewQuestion,hits:(signal.terms||[]).filter(function(term){return found(text,term);})};}),matched=sections.filter(function(section){return section.hits.length;});record={tool:'bill-analyzer',ruleVersion:version,createdAt:new Date().toISOString(),scope:'English-language lexical triage for six configured regulatory-language categories; no clause extraction, completeness, applicability, legal-effect, risk, or compliance determination.',input:text,outcome:matched.length?'review-signals-found':'no-configured-signal-abstention',sections:matched,limitations:${JSON.stringify(r.limitations || [])},humanReview:'Review the complete official text, definitions, cross-references, amendments, effective status, jurisdiction, and current authority with a licensed attorney or other qualified reviewer before reliance.'};var body=matched.map(function(section){return '<div><h3>'+esc(section.label)+'</h3><p>Triggered by: <strong>'+section.hits.map(esc).join(', ')+'</strong>.</p><p><strong>Review question:</strong> '+esc(section.reviewQuestion)+'</p></div>';}).join('');var boundary=matched.length?'Configured language signals found. They identify review candidates, not complete clauses or legal conclusions.':'No configured signal found. The tool abstains; this does not establish that the text contains no obligation, prohibition, deadline, threshold, authority, or enforcement provision.';shell.complete('<p class="eyebrow">Review record · rule '+esc(version)+'</p><h3>'+esc(c.outputTitle)+'</h3><p><strong>'+esc(boundary)+'</strong></p>'+body+'<h4>Human-review boundary</h4><p>'+esc(record.humanReview)+'</p>','Complete bounded review record generated. Verify it before reliance.');disable(false);}function md(){if(!record)return'';return ['# '+c.outputTitle,'','Rule version: '+record.ruleVersion,'Run date: '+record.createdAt,'Outcome: '+record.outcome,'Scope: '+record.scope,'','## Supplied material','',record.input,''].concat(record.sections.flatMap(function(section){return['## '+section.label,'','Triggered by: '+section.hits.join(', '),'Review question: '+section.reviewQuestion,''];}),record.sections.length?[]:['## Abstention','','No configured signal found. This is not evidence that the text contains none of the reviewed provision types.',''],['## Limitations',''].concat(record.limitations.map(function(item){return'- '+item;})),['','## Human review','',record.humanReview]).join('\n');}form.addEventListener('submit',function(event){event.preventDefault();run();});form.addEventListener('reset',function(){setTimeout(function(){record=null;disable(true);window.AlohaState.create({form:form,result:out,errors:'browser-tool-errors',status:'browser-tool-status'}).reset();},0);});document.getElementById('browser-tool-sample').onclick=function(){input.value=c.sample||'';run();};document.getElementById('browser-tool-copy').onclick=function(){window.AlohaActions.copyText(md(),{success:'Review record copied.'});};document.getElementById('browser-tool-md').onclick=function(){window.AlohaActions.download({content:md(),filename:'bill-analyzer-review.md',mimeType:'text/markdown;charset=utf-8',version:version,license:'Review draft; not legal advice.'});};document.getElementById('browser-tool-json').onclick=function(){window.AlohaActions.download({content:JSON.stringify(record,null,2),filename:'bill-analyzer-review.json',mimeType:'application/json',version:version,license:'Review draft; not legal advice.'});};disable(true);})();</script>`;
}

function evidenceExplainerSection(r, contract, ruleVersion) {
  return `<section class="section"><div class="wrap"><p class="eyebrow">Interactive tool</p><h2 class="h2">${escapeHtml(r.demo.outputTitle)}</h2><p class="muted">Deterministic rule version ${ruleVersion}. English-language claim triage; input is processed only in this browser.</p><noscript><div class="card"><strong>JavaScript is required to run this browser-local tool.</strong></div></noscript><div class="grid grid-2"><form id="browser-tool-form" class="card" novalidate><label for="browser-tool-input"><strong>${escapeHtml(r.demo.inputLabel)}</strong></label><textarea id="browser-tool-input" style="width:100%;min-height:14rem"></textarea><p><button class="btn btn--primary" type="submit">Run locally</button> <button class="btn btn--outline" type="button" id="browser-tool-sample">Load sample</button> <button class="btn btn--ghost" type="reset">Clear</button></p><div id="browser-tool-errors" class="card" role="alert" hidden></div></form><div class="card"><p class="eyebrow">Structured output</p><p id="browser-tool-status" class="muted" role="status" aria-live="polite">No result has been generated.</p><div id="browser-tool-output" hidden aria-live="polite"></div><p><button id="browser-tool-copy" class="btn btn--ghost" type="button" disabled>Copy result</button> <button id="browser-tool-md" class="btn btn--outline" type="button" disabled>Download Markdown</button></p></div></div></div></section><script>(function(){var c=${contract},version=${JSON.stringify(ruleVersion)},form=document.getElementById('browser-tool-form'),input=document.getElementById('browser-tool-input'),out=document.getElementById('browser-tool-output'),record=null;function esc(v){return window.AlohaState.escapeHtml(v);}function disable(v){document.getElementById('browser-tool-copy').disabled=v;document.getElementById('browser-tool-md').disabled=v;}function found(text,term){var escaped=String(term).replace(/[.*+?^\${}()|[\]\\]/g,'\\$&'),left=/^[a-z0-9]/i.test(term)?'\\b':'',right=/[a-z0-9]$/i.test(term)?'\\b':'';return new RegExp(left+escaped+right,'i').test(text);}function run(){var shell=window.AlohaState.create({form:form,result:out,errors:'browser-tool-errors',status:'browser-tool-status'}),text=input.value.trim();if(!text)return shell.invalid(['Enter one health claim before running the tool.']);var sections=(c.signals||[]).map(function(signal){return{label:signal.label,evidenceQuestion:signal.evidenceQuestion,evidenceDesign:signal.evidenceDesign,confounders:signal.confounders,hits:(signal.terms||[]).filter(function(term){return found(text,term);})};}),matches=sections.reduce(function(n,s){return n+s.hits.length;},0),matched=sections.filter(function(s){return s.hits.length;});record={tool:'evidence-explainer',ruleVersion:version,createdAt:new Date().toISOString(),scope:'English-language lexical triage into five configured claim-language categories; no truth, evidence-quality, literature-review, study-sufficiency, or medical determination.',input:text,outcome:matches?'review-signals-found':'no-configured-signal-abstention',sections:matched,limitations:${JSON.stringify(r.limitations || [])},humanReview:'Use the complete claim, relevant population and outcomes, retrieved primary literature, domain standards, and qualified human review before relying on or repeating the claim.'};var body=matched.map(function(s){return '<div><h3>'+esc(s.label)+'</h3><p>Triggered by: <strong>'+s.hits.map(esc).join(', ')+'</strong>.</p><p><strong>Evidence question:</strong> '+esc(s.evidenceQuestion)+'</p><p><strong>Evidence-design prompt:</strong> '+esc(s.evidenceDesign)+'</p><p><strong>Confounders to inspect:</strong> '+s.confounders.map(esc).join('; ')+'.</p></div>';}).join('');var boundary=matches?'Configured language signals found. The prompts frame questions; they do not grade evidence or determine truth.':'No configured signal found. The tool abstains; this does not establish that the claim is supported, accurate, or outside review.';shell.complete('<p class="eyebrow">Review record · rule '+esc(version)+'</p><h3>'+esc(c.outputTitle)+'</h3><p><strong>'+esc(boundary)+'</strong></p>'+body+'<h4>Human-review boundary</h4><p>'+esc(record.humanReview)+'</p>','Complete bounded review record generated. Verify it before reliance.');disable(false);}function md(){if(!record)return'';return ['# '+c.outputTitle,'','Rule version: '+record.ruleVersion,'Run date: '+record.createdAt,'Outcome: '+record.outcome,'Scope: '+record.scope,'','## Supplied material','',record.input,''].concat(record.sections.flatMap(function(s){return['## '+s.label,'','Triggered by: '+s.hits.join(', '),'Evidence question: '+s.evidenceQuestion,'Evidence-design prompt: '+s.evidenceDesign,'Confounders to inspect: '+s.confounders.join('; '),''];}),record.sections.length?[]:['## Abstention','','No configured signal found. This is not evidence that the claim is supported or accurate.',''],['## Limitations',''].concat(record.limitations.map(function(x){return'- '+x;})),['','## Human review','',record.humanReview]).join('\n');}form.addEventListener('submit',function(e){e.preventDefault();run();});form.addEventListener('reset',function(){setTimeout(function(){record=null;disable(true);window.AlohaState.create({form:form,result:out,errors:'browser-tool-errors',status:'browser-tool-status'}).reset();},0);});document.getElementById('browser-tool-sample').onclick=function(){input.value=c.sample||'';run();};document.getElementById('browser-tool-copy').onclick=function(){window.AlohaActions.copyText(md(),{success:'Review record copied.'});};document.getElementById('browser-tool-md').onclick=function(){window.AlohaActions.download({content:md(),filename:'evidence-explainer-review.md',mimeType:'text/markdown;charset=utf-8',version:version,license:'Evidence-literacy draft; not medical advice.'});};disable(true);})();</script>`;
}

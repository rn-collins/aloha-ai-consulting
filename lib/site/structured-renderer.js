...
function universityDetailExperience(r, related, registry) {
  ...
  const education = r.kind === 'course' ? renderEducationSystem(r.education, linkResources) : '';
  const facts = [
    ['Format', format],
...
function renderEducationSystem(education, linkResources) {
...
<div class="detail-dossier__body">${module.summary ? `<p>${linkResources(module.summary)}</p>` : ''}<ol>${lessonsFor(module).map((lesson) => `<li><h3>${escapeHtml(lesson.title)}</h3>${lesson.minutes ? `<p class="mini">${escapeHtml(String(lesson.minutes))} minutes</p>` : ''}${(lesson.content || []).map((paragraph) => `<p>${linkResources(paragraph)}</p>`).join('')}${lesson.practice ? `<p><strong>Practice:</strong> ${linkResources(lesson.practice)}</p>` : ''}</li>`).join('')}</ol></div>
</details>`).join('');
...
<p><a class="btn btn--primary" href="/tools/citation-verifier">Open the browser-local Citation Verifier <span aria-hidden="true">↗</span></a></p>
</div></section>`;
}

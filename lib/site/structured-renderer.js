...
<main id="main">
${hero(resource)}
${resource.pathname === '/' ? '' : identity(resource)}
...
function institutionalSection(r, include = () => true) {
  return (r.institutionalSections || []).filter(include).map((section) => `<section class="section"${section.id ? ` id="${escapeHtml(section.id)}"` : ''}><div class="wrap"><p class="eyebrow">${escapeHtml(section.eyebrow || 'Institution')}</p><h2 class="h2">${escapeHtml(section.title)}</h2>${section.intro ? `<p class="lead">${escapeHtml(section.intro)}</p>` : ''}<div class="grid grid-3">${(section.items || []).map((item) => {

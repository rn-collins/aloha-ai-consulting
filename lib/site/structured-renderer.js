...
export function renderStructuredPage({ resource, registry }) {
  ...
  const isResourceDetail = isEditorialDetail || isInteractiveDetail;
...
<p class="detail-cover__issue">${r.kind === 'monitor' ? 'LIVE SIGNAL DESK' : 'RESEARCH FILE'} · ${escapeHtml(maturityLabel(r.maturity))}</p>
<div class="detail-cover__type">${r.kind === 'monitor' ? '<span>WATCH</span><span>VERIFY</span><span>ACT</span>' : r.kind === 'assessment' ? '<span>ANSWER</span><span>SEE</span><span>DECIDE</span>' : r.kind === 'tool' ? '<span>INPUT</span><span>RUN</span><span>CHECK</span>' : '<span>QUESTION</span><span>EVIDENCE</span><span>METHOD</span>'}</div>
<div class="detail-cover__stamp">${r.kind === 'monitor' ? 'DATED<br><strong>NOT FINAL</strong>' : isInteractiveDetail && hasInteractiveWorkspace ? 'LOCAL<br><strong>BY DESIGN</strong>' : 'READ<br><strong>THE RECORD</strong>'}</div>
<p class="detail-cover__note">${r.kind === 'monitor' ? 'Signals change. Sources stay visible.' : isInteractiveDetail && hasInteractiveWorkspace ? 'Try the public version. Keep human judgment in the loop.' : isInteractiveDetail ? 'Inspect the method and maturity before treating this as usable.' : 'An argument you can inspect, not a claim you must trust.'}</p>
</div>` : '';
...
function interactiveDetailExperience(r, related, registry) {
  ...
  const facts = [
    ...
    ['Use', isAssessment ? 'Orient, then verify' : 'Generate, inspect, then verify']
  ];
...
function renderEditorialBlocks(blocks) {
  return blocks.map((block) => {
    if (block.type === 'paragraph') return `<p>${linkKnownResources(block.text)}</p>`;

...
function cta(r) {
  ...
  const learning = ['learningHub', 'course', 'lesson'].includes(r.kind);
...
function resolveRelated(resource, registry) {
  const relationships = resource.relationships || (resource.relatedIds || []).map((target) => ({ type: 'related_to', target }));
  ...
  for (const { resource: item } of items) {
    ...
    else if (['learningHub', 'course', 'lesson'].includes(item.kind)) groups['Related University Lessons'].push(item);
...
function label(kind) { return ({ service: 'Service', product: 'Product', learningHub: 'Learning hub', course: 'Course', lesson: 'Lesson', assessment: 'Assessment', tool: 'Tool', monitor: 'Monitor', research: 'Research', build: 'Build' })[kind] || 'Resource'; }
function buildSchema(r) { return { '@context': 'https://schema.org', '@type': r.kind === 'service' ? 'Service' : ['learningHub','course','lesson'].includes(r.kind) ? 'LearningResource' : 'CreativeWork', name: r.title, description: r.summary, url: `${BASE_URL}${r.pathname}`, provider: { '@type': 'Organization', name: 'Aloha AI' } }; }

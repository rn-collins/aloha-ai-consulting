...
<title>${escapeHtml(resource.seoTitle || resource.title)} | Aloha AI</title>
...
<meta property="og:title" content="${escapeHtml(resource.seoTitle || resource.title)}">
...
function hero(r) {
  const actions = Array.isArray(r.actions) && r.actions.length
    ? r.actions.map((action, index) => `<a class="btn ${index === 0 ? 'btn--primary' : 'btn--ghost'}" href="${escapeHtml(action.href)}">${escapeHtml(action.label)}</a>`).join('')
...
function editorial(r) {
  ...
  return introHtml + sections.map((section, index) => {
    ...
    return `<section class="section${index % 2 ? ' section--paper' : ''}"><div class="wrap prose">${section.eyebrow ? `<p class="eyebrow">${escapeHtml(section.eyebrow)}</p>` : ''}<h2 class="h2">${escapeHtml(section.title)}</h2>${blocks}</div></section>`;
  }).join('\n');
}
...
function institutionalSection(r) {
  return (r.institutionalSections || []).map((section) => `<section class="section"${section.id ? ` id="${escapeHtml(section.id)}"` : ''}><div class="wrap"><p class="eyebrow">${escapeHtml(section.eyebrow || 'Institution')}</p><h2 class="h2">${escapeHtml(section.title)}</h2>${section.intro ? `<p class="lead">${escapeHtml(section.intro)}</p>` : ''}<div class="grid grid-3">${(section.items || []).map((item) => {
    const body = `<h3>${escapeHtml(item.title)}</h3>${item.subtitle ? `<p class="mini">${escapeHtml(item.subtitle)}</p>` : ''}<p class="muted">${escapeHtml(item.text)}</p>${item.points?.length ? `<ul>${item.points.map((point) => `<li>${escapeHtml(point)}</li>`).join('')}</ul>` : ''}${item.href ? '<span class="mini">Explore →</span>' : ''}`;
    return item.href ? `<a class="card card--hover" href="${escapeHtml(item.href)}">${body}</a>` : `<div class="card">${body}</div>`;
...
function cta(r) {
  ...
  return `<section class="section page-cta"><div class="wrap page-cta__inner"><div><p class="eyebrow">Next step</p><h2 class="h2">${product ? 'Inspect the product before adopting it.' : learning ? 'Choose the next learning or implementation path.' : governance ? 'Inspect the governing record and connected practice.' : 'Start with the workflow, decision, or evidence problem.'}</h2></div><div class="page-actions"><a class="btn btn--primary" href="${learning ? `${r.pathname}#details` : governance ? '/about' : '/#start'}">${learning ? 'Start learning' : governance ? 'About the practice' : 'Start a conversation'}</a><a class="btn btn--outline" href="/methods">Inspect the methods</a></div></div></section>`;
}

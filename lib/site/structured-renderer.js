...
export function renderStructuredPage({ resource, registry }) {
  ...
  const isResourceDetail = isEditorialDetail || isMethodsDetail || isInteractiveDetail || isProductGovernanceDetail || isUniversityDetail || isUniversityServiceDetail || isUniversityInstitutional;
  const usesDetailHero = isResourceDetail || isServiceDetail || isResearchDetail;
...
${isUniversityServiceDetail ? universityServiceDetailExperience(resource, related, registry) : ''}
...
${isUniversityServiceDetail || ['/', '/university/contact', '/stacks'].includes(resource.pathname) ? '' : cta(resource)}
</main>
...
function hero(r) {
  ...
  const isResourceDetail = isEditorialDetail || isMethodsDetail || isInteractiveDetail || isServiceDetail || isResearchDetail || isProductGovernanceDetail || isUniversityDetail || isUniversityServiceDetail || isUniversityInstitutional;
...
function universityServiceDetailExperience(r, related, registry) {
  ...
  const bookingHref = 'https://outlook.office.com/bookwithme/user/3ff7c8299177431a8fc2b099238c5e43@northeastern.edu?anonymous';
...
<section class="service-close"><div class="wrap wrap--wide"><p class="service-kicker">Next step</p><h2>Bring the workflow<br>that keeps getting stuck.</h2><p>We will tell you honestly whether this service is the right first move.</p><div><a class="btn btn--primary" href="${bookingHref}" target="_blank" rel="noopener">Start a conversation <span aria-hidden="true">↗</span></a><a class="btn btn--ghost" href="/university/services">Compare all services</a></div></div></section>
...
function universityInstitutionalExperience(r, related, registry) {
  ...
  const isServices = r.pathname === '/university/services';
  const orientation = isStart
    ? 'Choose the closest starting point. You can change paths whenever the work changes.'
    : isServices
      ? 'Learning is free. Paid support is a separate choice with an explicit scope.'
      : 'The teaching standard, evidence boundary, and professional limits belong on the same page.';
  const facts = [
    ['Page', isStart ? 'Learning orientation' : isServices ? 'Support options' : 'Teaching standard'],
...
<div class="detail-dossier__body">${section.eyebrow ? `<p class="eyebrow">${escapeHtml(section.eyebrow)}</p>` : ''}${renderEditorialBlocks(section.blocks || [], linkResources)}</div>
  ...
  return `<section class="detail-orientation learning-orientation"><div class="wrap wrap--wide">
<div class="detail-orientation__intro"><p class="eyebrow">${isStart ? 'Find your first step' : isServices ? 'Know what is free and what is paid' : 'Read the teaching contract'}</p>${intro.length ? intro.map((paragraph) => `<p>${linkResources(paragraph)}</p>`).join('') : `<p>${linkResources(r.summary)}</p>`}<p class="interactive-boundary">${linkResources(orientation)}</p></div>
<dl class="detail-facts">${facts.map(([term, value]) => `<div><dt>${escapeHtml(term)}</dt><dd>${escapeHtml(value)}</dd></div>`).join('')}</dl>
</div></section>
...
<div class="detail-section-head"><div><p class="eyebrow">${isStart ? 'The route map' : isServices ? 'The support record' : 'The teaching record'}</p><h2>${isStart ? 'Choose by need, not by tool.' : isServices ? 'Know what help means before you ask for it.' : 'Trust the method you can inspect.'}</h2></div><p>${sections.length} numbered section${sections.length === 1 ? '' : 's'}. Named tools and Aloha AI resources link directly to their destinations.</p></div>
...
function cta(r) {
  ...
  const primaryHref = isContact
    ...
    : ['/university/about', '/university/start-here', '/university/services'].includes(r.pathname)

...
${isEditorialDetail || ['/', '/services', '/builds', '/university', '/university/contact', '/methods', '/about'].includes(resource.pathname) ? '' : identity(resource)}
${isEditorialDetail ? '' : prioritySections(resource)}
${isEditorialDetail ? '' : resource.pathname === '/services' ? servicesDepth(resource) : resource.pathname === '/builds' ? buildsDepth(resource) : resource.pathname === '/university' ? universityDepth(resource) : resource.pathname === '/university/contact' ? '' : editorial(resource)}
${isEditorialDetail || ['/', '/university/contact'].includes(resource.pathname) ? '' : kindSections(resource, registry)}
${isEditorialDetail || ['/', '/university/contact'].includes(resource.pathname) ? '' : evidence(resource)}
${isEditorialDetail || ['/', '/university/contact'].includes(resource.pathname) ? '' : method(resource)}
${isEditorialDetail || ['/', '/university/contact'].includes(resource.pathname) ? '' : governance(resource)}
${isEditorialDetail || ['/', '/builds', '/university', '/university/contact'].includes(resource.pathname) ? '' : relatedSections(related)}
...
</div>` : isEditorialDetail ? `<div class="detail-cover detail-cover--${escapeHtml(r.kind)}" aria-hidden="true">
<p class="detail-cover__issue">${r.kind === 'monitor' ? 'LIVE SIGNAL DESK' : 'RESEARCH FILE'} · ${escapeHtml(maturityLabel(r.maturity))}</p>
<div class="detail-cover__type">${r.kind === 'monitor' ? '<span>WATCH</span><span>VERIFY</span><span>ACT</span>' : '<span>QUESTION</span><span>EVIDENCE</span><span>METHOD</span>'}</div>
<div class="detail-cover__stamp">${r.kind === 'monitor' ? 'DATED<br><strong>NOT FINAL</strong>' : 'READ<br><strong>THE RECORD</strong>'}</div>
<p class="detail-cover__note">${r.kind === 'monitor' ? 'Signals change. Sources stay visible.' : 'An argument you can inspect, not a claim you must trust.'}</p>
</div>` : '';
  return `<section class="page-hero section--ink${isHome ? ' page-hero--home' : isServices ? ' page-hero--services' : isBuilds ? ' page-hero--builds' : isUniversity ? ' page-hero--university' : isContact ? ' page-hero--contact' : isEditorialDetail ? ' page-hero--detail' : ''}"><div class="wrap page-hero__inner">

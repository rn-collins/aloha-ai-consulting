...
const EXTERNAL_TOOL_LINKS = Object.freeze({
  ...
  'Claude for Work': 'https://www.anthropic.com/claude-for-enterprise',
...
export function renderStructuredPage({ resource, registry }) {
  ...
  const isUniversityDetail = ['lesson', 'course', 'playbook', 'template', 'toolGuide'].includes(resource.kind);
  const isResourceDetail = isEditorialDetail || isInteractiveDetail || isProductGovernanceDetail || isUniversityDetail;
...
<body class="${resource.pathname === '/' ? 'is-home' : resource.pathname === '/services' ? 'is-services' : resource.pathname === '/builds' ? 'is-builds' : resource.pathname === '/university' ? 'is-university' : resource.pathname === '/university/contact' ? 'is-contact' : isResourceDetail ? `is-resource-detail is-${resource.kind}-detail` : ''}" data-resource-id="${escapeHtml(resource.id)}" data-resource-kind="${escapeHtml(resource.kind)}" data-maturity="${escapeHtml(resource.maturity)}">
<a class="skip" href="#main">Skip to content</a>
<header class="nav"></header>
<main id="main">
${hero(resource)}
${isEditorialDetail ? resourceDetailExperience(resource, related) : ''}
...
${['/builds', '/university'].includes(resource.pathname) ? '' : priorityCollection(resource, registry)}
${engagementPortfolio(resource)}
${isResourceDetail || ['/', '/services', '/builds', '/university', '/university/contact', '/methods', '/about'].includes(resource.pathname) ? '' : identity(resource)}
${isResourceDetail ? '' : prioritySections(resource)}
${isResourceDetail ? '' : resource.pathname === '/services' ? servicesDepth(resource) : resource.pathname === '/builds' ? buildsDepth(resource) : resource.pathname === '/university' ? universityDepth(resource) : resource.pathname === '/university/contact' ? '' : editorial(resource)}
${isResourceDetail || ['/', '/university/contact'].includes(resource.pathname) ? '' : kindSections(resource, registry)}
${isResourceDetail || ['/', '/university/contact'].includes(resource.pathname) ? '' : evidence(resource)}
${isResourceDetail || ['/', '/university/contact'].includes(resource.pathname) ? '' : method(resource)}
${isResourceDetail || ['/', '/university/contact'].includes(resource.pathname) ? '' : governance(resource)}
${isResourceDetail || ['/', '/builds', '/university', '/university/contact'].includes(resource.pathname) ? '' : relatedSections(related)}
${['/', '/university/contact'].includes(resource.pathname) ? '' : cta(resource)}
</main>
...
function hero(r) {
  ...
  const isUniversityDetail = ['lesson', 'course', 'playbook', 'template', 'toolGuide'].includes(r.kind);
  const isResourceDetail = isEditorialDetail || isInteractiveDetail || isProductGovernanceDetail || isUniversityDetail;
  ...
  const actions = Array.isArray(r.actions) && r.actions.length
    ...
    : isInteractiveDetail && hasInteractiveWorkspace
      ...
      : isEditorialDetail
      ? `<a class="btn btn--primary" href="#detail-record">${r.kind === 'monitor' ? 'Open the signal record' : 'Read the research record'}</a><a class="btn btn--ghost" href="${r.kind === 'monitor' ? '/monitors' : '/research'}">Browse the collection</a>`
      ...
      : isUniversityDetail
        ? `<a class="btn btn--primary" href="#learning-record">${r.kind === 'lesson' ? 'Start the lesson' : r.kind === 'course' ? 'Open the course' : r.kind === 'playbook' ? 'Open the playbook' : r.kind === 'template' ? 'Open the template' : 'Open the guide'}</a><a class="btn btn--ghost" href="${r.kind === 'lesson' ? '/university/learn' : r.kind === 'course' ? '/university/courses' : r.kind === 'playbook' ? '/university/playbooks' : r.kind === 'template' ? '/university/templates' : '/university/tools'}">Browse ${r.kind === 'toolGuide' ? 'tool guides' : `${r.kind}s`}</a>`
...
</div>` : isResourceDetail ? `<div class="detail-cover detail-cover--${escapeHtml(r.kind)}" aria-hidden="true">
<p class="detail-cover__issue">${r.kind === 'monitor' ? 'LIVE SIGNAL DESK' : r.kind === 'tool' ? 'TOOL RECORD' : r.kind === 'assessment' ? 'ASSESSMENT RECORD' : r.kind === 'product' ? 'PRODUCT SYSTEM' : r.kind === 'policy' ? 'POLICY RECORD' : r.pathname === '/partners' ? 'PARTNERSHIP RECORD' : r.kind === 'institutional' ? 'PRACTICE RECORD' : r.kind === 'lesson' ? 'UNIVERSITY LESSON' : r.kind === 'course' ? 'UNIVERSITY COURSE' : r.kind === 'playbook' ? 'UNIVERSITY PLAYBOOK' : r.kind === 'template' ? 'UNIVERSITY TEMPLATE' : r.kind === 'toolGuide' ? 'UNIVERSITY TOOL GUIDE' : 'RESEARCH FILE'} · ${escapeHtml(maturityLabel(r.maturity))}</p>
<div class="detail-cover__type">${r.kind === 'monitor' ? '<span>WATCH</span><span>VERIFY</span><span>ACT</span>' : r.kind === 'assessment' ? '<span>ANSWER</span><span>SEE</span><span>DECIDE</span>' : r.kind === 'tool' ? '<span>INPUT</span><span>RUN</span><span>CHECK</span>' : r.kind === 'product' ? '<span>GROUND</span><span>BUILD</span><span>MAINTAIN</span>' : r.kind === 'policy' ? '<span>READ</span><span>KNOW</span><span>RETURN</span>' : r.pathname === '/partners' ? '<span>RIGHTS</span><span>ROLES</span><span>PATH</span>' : r.kind === 'institutional' ? '<span>RECORD</span><span>ROLE</span><span>LIMIT</span>' : r.kind === 'lesson' ? '<span>LEARN</span><span>TRY</span><span>CHECK</span>' : r.kind === 'course' ? '<span>PLAN</span><span>BUILD</span><span>TEST</span>' : r.kind === 'playbook' ? '<span>FOLLOW</span><span>MAKE</span><span>RUN</span>' : r.kind === 'template' ? '<span>COPY</span><span>ADAPT</span><span>OWN</span>' : r.kind === 'toolGuide' ? '<span>COMPARE</span><span>TEST</span><span>CHOOSE</span>' : '<span>QUESTION</span><span>EVIDENCE</span><span>METHOD</span>'}</div>
...
<div class="university-depth__head"><div><p class="eyebrow">How the school works</p><h2>Want the teaching method and proof?</h2></div><p>The paths above are enough to begin. Open these notes when you want the philosophy, evidence standard, service boundary, or full program detail.</p></div>
<div class="university-depth__sections">${sections.map((section, index) => `<details class="university-depth__item">
...
<div class="detail-section-head"><div><p class="eyebrow">Continue through the system</p><h2>Follow the evidence outward.</h2></div><p>Every named destination is a direct link to its canonical page.</p></div>
...
<section class="detail-dossier ${isProduct ? 'product-record' : 'governance-record'}" id="detail-record"><div class="wrap wrap--wide">
...
<div class="detail-dossier__body">${section.eyebrow ? `<p class="eyebrow">${escapeHtml(section.eyebrow)}</p>` : ''}${renderEditorialBlocks(section.blocks || [], linkResources)}</div>
  ...
  const nextHref = r.kind === 'lesson' ? '/university/learn' : r.kind === 'course' ? '/university/courses' : r.kind === 'playbook' ? '/university/playbooks' : r.kind === 'template' ? '/university/templates' : '/university/tools';
  const plural = r.kind === 'toolGuide' ? 'tool guides' : `${r.kind}s`;
...
function homeEditorial(intro, sections) {
  const introHtml = intro.length
    ? `<section class="home-editorial-intro"><div class="wrap wrap--wide"><p class="eyebrow">In depth</p><div>${intro.map((paragraph) => `<p>${linkKnownResources(paragraph)}</p>`).join('')}</div></div></section>`
    : '';
  return introHtml + sections.map((section, index) => {
    const items = [];
    for (let blockIndex = 0; blockIndex < (section.blocks || []).length; blockIndex += 1) {
      ...
      if (block.type === 'heading') {
        const title = block.href
          ? `<a href="${escapeHtml(block.href)}">${escapeHtml(block.text)} <span aria-hidden="true">↗</span></a>`
          : escapeHtml(block.text);
        const description = next?.type === 'paragraph' ? `<p>${linkKnownResources(next.text)}</p>` : '';
        if (description) blockIndex += 1;
        items.push(`<article class="home-editorial-card"><h3>${title}</h3>${description}</article>`);
      } else if (block.type === 'links') {
        items.push(`<nav class="home-editorial-links" aria-label="${escapeHtml(section.title)} links">${(block.items || []).map((item) => `<a href="${escapeHtml(item.href)}">${escapeHtml(item.label)} <span aria-hidden="true">↗</span></a>`).join('')}</nav>`);
      } else {
        items.push(`<div class="home-editorial-copy">${renderEditorialBlocks([block])}</div>`);
      }
    }
    const sectionTitle = section.href
      ? `<a href="${escapeHtml(section.href)}">${escapeHtml(section.title)} <span aria-hidden="true">↗</span></a>`
      : escapeHtml(section.title);
    return `<section class="home-editorial home-editorial--${index + 1}"><div class="wrap wrap--wide"><div class="home-editorial__head"><p class="eyebrow">${escapeHtml(section.eyebrow || 'Aloha AI')}</p><h2>${sectionTitle}</h2></div><div class="home-editorial__grid">${items.join('')}</div></div></section>`;
  }).join('');
}

...
function institutionalSection(r) {
  return (r.institutionalSections || []).map((section) => `<section class="section"><div class="wrap"><p class="eyebrow">${escapeHtml(section.eyebrow || 'Institution')}</p><h2 class="h2">${escapeHtml(section.title)}</h2>${section.intro ? `<p class="lead">${escapeHtml(section.intro)}</p>` : ''}<div class="grid grid-3">${(section.items || []).map((item) => `<div class="card"><h3>${escapeHtml(item.title)}</h3>${item.subtitle ? `<p class="mini">${escapeHtml(item.subtitle)}</p>` : ''}<p class="muted">${escapeHtml(item.text)}</p>${item.points?.length ? `<ul>${item.points.map((point) => `<li>${escapeHtml(point)}</li>`).join('')}</ul>` : ''}</div>`).join('')}</div></div></section>`).join('\n');
}

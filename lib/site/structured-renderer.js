...
export function renderStructuredPage({ resource, registry }) {
  ...
  const isResourceDetail = isEditorialDetail || isMethodsDetail || isInteractiveDetail || isProductGovernanceDetail || isUniversityDetail || isUniversityInstitutional;
...
${['/', '/university/contact', '/stacks'].includes(resource.pathname) ? '' : cta(resource)}
</main>
...
function hero(r) {
  ...
  const isResourceDetail = isEditorialDetail || isMethodsDetail || isInteractiveDetail || isServiceDetail || isResearchDetail || isProductGovernanceDetail || isUniversityDetail || isUniversityInstitutional;
...
<p class="detail-cover__issue">${isMethodsDetail ? 'METHODS STANDARD' : isUniversityInstitutional ? 'UNIVERSITY GUIDE' : r.kind === 'monitor' ? 'LIVE SIGNAL DESK' : r.kind === 'tool' ? 'TOOL RECORD' : r.kind === 'assessment' ? 'ASSESSMENT RECORD' : r.kind === 'service' ? 'SERVICE RECORD' : r.kind === 'product' ? 'PRODUCT SYSTEM' : r.kind === 'policy' ? 'POLICY RECORD' : r.pathname === '/partners' ? 'PARTNERSHIP RECORD' : r.kind === 'institutional' ? 'PRACTICE RECORD' : r.kind === 'lesson' ? 'UNIVERSITY LESSON' : r.kind === 'course' ? 'UNIVERSITY COURSE' : r.kind === 'playbook' ? 'UNIVERSITY PLAYBOOK' : r.kind === 'template' ? 'UNIVERSITY TEMPLATE' : r.kind === 'toolGuide' ? 'UNIVERSITY TOOL GUIDE' : r.kind === 'useCase' ? 'UNIVERSITY USE CASE' : 'RESEARCH FILE'} · ${escapeHtml(maturityLabel(r.maturity))}</p>
<div class="detail-cover__type">${isMethodsDetail ? '<span>DEFINE</span><span>TEST</span><span>MAINTAIN</span>' : isUniversityInstitutional ? '<span>ORIENT</span><span>CHOOSE</span><span>BEGIN</span>' : r.kind === 'monitor' ? '<span>WATCH</span><span>VERIFY</span><span>ACT</span>' : r.kind === 'assessment' ? '<span>ANSWER</span><span>SEE</span><span>DECIDE</span>' : r.kind === 'tool' ? '<span>INPUT</span><span>RUN</span><span>CHECK</span>' : r.kind === 'service' ? '<span>SCOPE</span><span>BUILD</span><span>HAND OFF</span>' : r.kind === 'product' ? '<span>GROUND</span><span>BUILD</span><span>MAINTAIN</span>' : r.kind === 'policy' ? '<span>READ</span><span>KNOW</span><span>RETURN</span>' : r.pathname === '/partners' ? '<span>RIGHTS</span><span>ROLES</span><span>PATH</span>' : r.kind === 'institutional' ? '<span>RECORD</span><span>ROLE</span><span>LIMIT</span>' : r.kind === 'lesson' ? '<span>LEARN</span><span>TRY</span><span>CHECK</span>' : r.kind === 'course' ? '<span>PLAN</span><span>BUILD</span><span>TEST</span>' : r.kind === 'playbook' ? '<span>FOLLOW</span><span>MAKE</span><span>RUN</span>' : r.kind === 'template' ? '<span>COPY</span><span>ADAPT</span><span>OWN</span>' : r.kind === 'toolGuide' ? '<span>COMPARE</span><span>TEST</span><span>CHOOSE</span>' : r.kind === 'useCase' ? '<span>CHOOSE</span><span>DRAFT</span><span>REVIEW</span>' : '<span>QUESTION</span><span>EVIDENCE</span><span>METHOD</span>'}</div>

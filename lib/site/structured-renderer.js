...
function cta(r) {
  ...
  const learning = ['learningHub', 'course', 'lesson', 'playbook', 'template'].includes(r.kind);
...
function resolveRelated(resource, registry) {
  ...
  for (const { resource: item } of items) {
    ...
    else if (['learningHub', 'course', 'lesson', 'playbook', 'template'].includes(item.kind)) groups['Related University Lessons'].push(item);
...
function label(kind) { return ({ service: 'Service', product: 'Product', learningHub: 'Learning hub', course: 'Course', lesson: 'Lesson', playbook: 'Playbook', template: 'Template', assessment: 'Assessment', tool: 'Tool', monitor: 'Monitor', research: 'Research', build: 'Build' })[kind] || 'Resource'; }
function buildSchema(r) { return { '@context': 'https://schema.org', '@type': r.kind === 'service' ? 'Service' : ['learningHub','course','lesson','playbook','template'].includes(r.kind) ? 'LearningResource' : 'CreativeWork', name: r.title, description: r.summary, url: `${BASE_URL}${r.pathname}`, provider: { '@type': 'Organization', name: 'Aloha AI' } }; }

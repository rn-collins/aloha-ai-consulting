...
export function renderStructuredPage({ resource, registry }) {
  ...
  const isResourceDetail = isEditorialDetail || isInteractiveDetail || isProductGovernanceDetail;
...
<p class="detail-cover__issue">${r.kind === 'monitor' ? 'LIVE SIGNAL DESK' : r.kind === 'tool' ? 'TOOL RECORD' : r.kind === 'assessment' ? 'ASSESSMENT RECORD' : r.kind === 'product' ? 'PRODUCT SYSTEM' : r.kind === 'policy' ? 'POLICY RECORD' : r.kind === 'institutional' ? 'PRACTICE RECORD' : 'RESEARCH FILE'} · ${escapeHtml(maturityLabel(r.maturity))}</p>
<div class="detail-cover__type">${r.kind === 'monitor' ? '<span>WATCH</span><span>VERIFY</span><span>ACT</span>' : r.kind === 'assessment' ? '<span>ANSWER</span><span>SEE</span><span>DECIDE</span>' : r.kind === 'tool' ? '<span>INPUT</span><span>RUN</span><span>CHECK</span>' : r.kind === 'product' ? '<span>GROUND</span><span>BUILD</span><span>MAINTAIN</span>' : r.kind === 'policy' ? '<span>READ</span><span>KNOW</span><span>RETURN</span>' : r.kind === 'institutional' ? '<span>RECORD</span><span>ROLE</span><span>LIMIT</span>' : '<span>QUESTION</span><span>EVIDENCE</span><span>METHOD</span>'}</div>
...
function productGovernanceDetailExperience(r, related, registry) {
  ...
  const facts = isProduct
    ...
    : [
        ['Record', isPolicy ? 'Public policy' : 'Institutional statement'],
        ['Status', maturityLabel(r.maturity)],
  ...
  const dossierItems = isProduct
    ...
    : isPolicy
      ? policySections.map((section) => ({
          title: `${Number.isFinite(section.order) ? `${section.order} · ` : ''}${section.title}`,
...
<div class="detail-dossier__body">${section.eyebrow ? `<p class="eyebrow">${escapeHtml(section.eyebrow)}</p>` : ''}${section.html}</div>
  ...
  return `<section class="detail-orientation"><div class="wrap wrap--wide">
<div class="detail-orientation__intro"><p class="eyebrow">${isProduct ? 'Before you adopt it' : isPolicy ? 'Before you use the site' : 'Before you rely on the profile'}</p>${intro.length ? intro.map((paragraph) => `<p>${linkResources(paragraph)}</p>`).join('') : `<p>${linkResources(r.summary)}</p>`}<p class="interactive-boundary">${linkResources(statusBoundary)}</p></div>
<dl class="detail-facts">${facts.map(([term, value]) => `<div><dt>${escapeHtml(term)}</dt><dd>${escapeHtml(value)}</dd></div>`).join('')}</dl>
</div></section>
<section class="detail-dossier ${isProduct ? 'product-record' : 'governance-record'}" id="detail-record"><div class="wrap wrap--wide">
<div class="detail-section-head"><div><p class="eyebrow">${isProduct ? 'The system record' : isPolicy ? 'The governing terms' : 'The institutional record'}</p><h2>${isProduct ? 'Architecture, status, and evidence in one place.' : isPolicy ? 'Open the section you need.' : 'Claims and boundaries stay together.'}</h2></div><p>${dossierItems.length} documented section${dossierItems.length === 1 ? '' : 's'}. Named Aloha AI resources link directly to their canonical pages.</p></div>
<div class="detail-dossier__list">${dossier || '<p>No extended record has been published yet.</p>'}</div>
</div></section>
<section class="detail-accountability"><div class="wrap wrap--wide">
...
function renderEditorialBlocks(blocks, linkText = linkKnownResources) {
  return blocks.map((block) => {
    ...
    if (block.type === 'heading') return `<h3>${block.href ? `<a href="${escapeHtml(block.href)}">${escapeHtml(block.text)} <span aria-hidden="true">↗</span></a>` : escapeHtml(block.text)}</h3>`;
...
function linkKnownResources(value) {
  ...
  for (const [name, href] of Object.entries(NAMED_RESOURCE_LINKS).sort(([a], [b]) => b.length - a.length)) {
    html = html.split(escapeHtml(name)).join(`<a href="${href}">${escapeHtml(name)}</a>`);
  }
  return html;
}
